...
> **版本 2.2.0** — 项目全面分析：严重等级对齐、工作流健壮性增强、CI/CD 加固、文档修复
...
2. **增量模式**（`--incremental`）：
   1. **检查 git 仓库**：运行 `git rev-parse --git-dir`，失败则输出错误信息 "错误：--incremental 需要 git 仓库，已回退到全量模式"，然后跳过增量流程继续全量审计
   2. **解析 `--since`**：
       - 如指定 `--since`：先尝试解析为日期（`git log --before=<date>`），再尝试 commit hash/HEAD~N
       - 若所有解析方式均失败：输出错误 "警告：无法解析 --since 参数 '<value>'，已回退到全量模式"，跳过增量流程
   3. **未指定 `--since`**：读取 `.audit-history.yaml`
       - 文件不存在 → 输出提示 "未找到审计历史记录，回退到全量模式"，跳过增量流程
       - 文件存在但解析失败或格式错误 → 输出警告 "审计历史文件解析失败，回退到全量模式"，跳过增量流程
       - 文件存在且格式正确，但最后一笔记录无 commit hash → 输出警告 "上次审计记录缺少 commit hash，无法确定增量起点，回退到全量模式"，跳过增量流程
   4. 无历史记录或无有效 since：回退到全量模式并提示用户
   5. 运行 `git diff --name-only <since> HEAD` 获取变更文件列表
       - 如果 `<since>` 超出 git reflog 范围（git 返回错误），捕获错误并输出 "警告：增量起点 <since> 超出 git 历史范围，尝试 `git log --all`"，尝试在全部引用中搜索；仍失败则回退到全量模式
   6. 过滤出仍存在于工作树的文件（检查 `git cat-file -e HEAD:<path>` 或 `test -f <path>`）
...
- 每个低危（Low）-1 分
- 信息（Info）不扣分
- 增量模式的分数按比例修正：`修正分 = 原始分 × (1 - 0.3 × (审计文件数 / 总文件数))`，覆盖文件少不等于更健康

输出趋势表格：
| 轮次 | 日期 | 发现总数 | 严重 | 高危 | 中危 | 低危 | 信息 | 健康分 | 增量/全量 |
|------|------|---------|------|------|------|------|------|--------|----------|
| 1 | 2025-06-01 | 23 | 3 | 8 | 7 | 3 | 2 | 42 | 全量 |
| 2 | 2025-06-15 | 18 | 2 | 5 | 6 | 3 | 2 | 58 | 增量 |
| 3 | 2025-07-01 | 12 | 1 | 3 | 4 | 3 | 1 | 76 | 增量 |
...
| 🟠 高危 | 显著影响代码质量或可用性，可能导致性能退化、维护困难、部分功能异常 | 1周内 |
| 🟡 中危 | 轻微违反最佳实践或编码规范，不影响功能正确性 | 1月内 |
| 🔵 低危 | 观察性发现，无直接风险，供参考或未来改进 | 酌情 |
| ⚪ 信息 | 纯信息性提示，不构成风险 | 无需响应 |
...
- **低危也值得关注**：低危项累积会显著降低代码可维护性，建议在常规开发中逐步清理
